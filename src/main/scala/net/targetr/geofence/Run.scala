...
object Run {
  ...
  def cartesianProduct[T](lst: Array[List[T]]): List[List[T]] = {
  ...
  def gapper(data: Array[Array[Int]]) = {
    val mins = data.reduce((a, v) => a.zip(v).map(i => math.min(i._1, i._2)))
    val maxs = data.reduce((a, v) => a.zip(v).map(i => math.max(i._1, i._2)))

    val mm = mins.zip(maxs).map{case (mi,mx) => (mi to mx).map(i => i)}

    cartesianProduct(mins.zip(maxs).map{case (mi,mx) => (mi to mx).map(i => i)}.map(_.toList))
  }
  ...
  def model(data: String, sensors: String) = {
    ...
    //val all = readings.filter(a => sense.contains(a._1.toInt)).map(a => a._2.map(v => v._1.toDouble +: sense(a._1.toInt) :+ v._2)).reduce((_,v) => v).flatten.toArray
    ...
    val y = all.map(_(6))
    //val train = readings.filter(a => sense.contains(a._1.toInt)).map(a => a._2.map(v => v._1.toDouble +: sense(a._1.toInt))).reduce((_,v) => v)
    //val label = readings.filter(a => sense.contains(a._1.toInt)).map(a => a._2.map(v => (v._2 * 100).toInt)).reduce((_,v) => v)
    //val df = DataFrame.of(all, "hour", "dow", "building", "floor", "bank", "desk", "util")
    //val df = DataFrame.of(all, "hour", "dow", "floor", "bank", "desk", "util")
    ...
    //every.map(v => v -> gMap.getOrElse(v, rf.predict(Array(v._1.toDouble,v._2.toDouble,v._3.toDouble,v._4.toDouble,v._5.toDouble)))).toMap
    every.map(v => v -> {val k = Array(v._1.toDouble,v._2.toDouble,v._3.toDouble,v._4.toDouble,v._5.toDouble); val py = rf.predict(k); (gMap.getOrElse(v,py), py)}).map{case (k,v) => k.productIterator.map(_.asInstanceOf[Int]).toArray ++ v.productIterator.map(_.asInstanceOf[Double]).toArray}.toArray
    //every.map(v => (v._1.toDouble,v._2.toDouble,v._3.toDouble,v._4.toDouble,v._5.toDouble)).map(k => (k, rf.predict(k))).map{case (k, py) => k :+ gMap.getOrElse(k,py) :+ py}.toMap
  }
